...
const { ignoreFolders, ignoreExtensions, ignoreFiles } = loadConfig()
const IGNORE_FOLDERS    = new Set(ignoreFolders)
const IGNORE_EXTENSIONS = new Set(ignoreExtensions)
const IGNORE_FILES      = new Set(ignoreFiles)
  ...
  const entries = fs
    ...
    .filter(entry => {
      if (entry.isDirectory()) {
        ...
        return !IGNORE_EXTENSIONS.has(ext) && !IGNORE_FILES.has(entry.name)
      }
    })
